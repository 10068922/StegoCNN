...
class PlotData(object):
    ...
    def plot_model_bis(self, model_details, path_to_save):
        ...
        fig, axs = plt.subplots(1, 2, figsize=(15, 5))
        # Summarize history for accuracy
        axs[0].plot(range(1, len(model_details['accuracy']) + 1),
                    model_details['accuracy'], color='red', linestyle='-',  linewidth=2.0)
        axs[0].plot(range(1, len(model_details['val_accuracy']) + 1),
                    model_details['val_accuracy'], color='blue', linestyle='-',  linewidth=2.0)
